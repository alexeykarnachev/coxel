...
uniform float surface_noise_ampl_mult = 0.1;
uniform float surface_noise_freq_init = 6.0;
uniform float surface_noise_mult = 0.2;
...
void main(void) {
    ...
    vec4 model_pos = vec4(p, 1.0);
    vec4 world_pos = world_mat * model_pos;
    vec4 proj_pos = proj_mat * view_mat * world_pos;
    vec2 tex_pos = 0.5 * (proj_pos.xy / proj_pos.w + 1.0);
    tese_out.model_pos = model_pos;
    tese_out.world_pos = world_pos;
    tese_out.proj_pos = proj_pos;
    tese_out.tex_pos = tex_pos;
