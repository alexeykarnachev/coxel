...
void _render_meshes() {
    ...
    for (size_t i = 0; i < N_CAMERA_ENTITIES; ++i) {
        ...
        program_set_uniform_matrix_4fv(
            ...
            camera_get_view_mat(transformation).data, 1, 1);
        program_set_uniform_matrix_4fv(
            program, "camera.proj_mat",
            camera_get_perspective_projection_mat(camera), 1, 1);
    ...
    for (size_t i = 0; i < N_POINT_LIGHT_ENTITIES; ++i) {
        ...
        program_set_uniform_4fv(
            program, "point_lights[0].world_pos",
            transformation->position.data, 1);
        program_set_uniform_4fv(
            program, "point_lights[0].color",
            point_light->color.data, 1);
        program_set_uniform_1f(
            program, "point_lights[0].energy",
            point_light->energy);
        
        // TODO: Currently I handle only one point light: the first one in the
        // POINT_LIGHT_ENTITIES array. I need to handle another ones if exist.
        // Make uniform name dynamic, e.g. "point_lights[i]".
        // Also, pass the correct `n_point_lights` uniform (below).
        break;
    }
    program_set_uniform_1i(program, "n_point_lights", 1);
