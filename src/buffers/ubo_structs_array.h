...
bool ubo_structs_array_create(
    ...
    glBufferData(GL_UNIFORM_BUFFER, struct_size * max_n_structs + 16, NULL, usage);
...
bool ubo_structs_array_add(UBOStructsArray* ubo, float packed_struct[]) {
    ...
    glBindBuffer(GL_UNIFORM_BUFFER, ubo->ubo);
    glBufferSubData(GL_UNIFORM_BUFFER, struct_idx * ubo->struct_size, ubo->struct_size, packed_struct);
    glBufferSubData(GL_UNIFORM_BUFFER, ubo->max_n_structs * ubo->struct_size, 16, &ubo->n_structs);
    glBindBuffer(GL_UNIFORM_BUFFER, 0);
