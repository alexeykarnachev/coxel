...
static Vec4 BUTTON_COLOR = {{0.2, 0.2, 0.2, 1.0}}; 
...
static size_t create_rect(
    ...
    ecs_add_component(
        pane, GUI_RECT_T,
        gui_rect_create(width, height, color)
    );
    ecs_add_component(
        pane, TRANSFORMATION_T,
        transformation_create(
            vec3_ones, vec3_zeros,
            vec3(x, y, 0)
        ) 
    );
...
static size_t create_text(
    ...
    ecs_add_component(
        text, GUI_TEXT_T, gui_text_create(label, font_size));
    ecs_add_component(
        text, TRANSFORMATION_T,
        transformation_create(
            vec3_ones, vec3_zeros,
            vec3(x, y, 0.0)
        ) 
...
static size_t create_button(
    ...
    size_t button = create_rect(
        parent, x, y, width, height, BUTTON_COLOR);
